...
socketxx::simple_socket_client<socketxx::base_netsock> iosl_master::slave_connect (std::string slave_id, in_port_t default_port, timeval timeout) {
	...
	auto sock = socketxx::simple_socket_client<socketxx::base_netsock> (addr, timeout);
...
socketxx::base_netsock iosl_master::slave_api_service_connect (std::string slave_id, std::string master_id, std::string api_service, timeval timeout) {
	try {
		socketxx::simple_socket_client<socketxx::base_netsock> slave_sock = iosl_master::slave_connect(slave_id, 0, timeout);
...
bool iosl_master::slave_test (std::string slave_id) {
	try {
		socketxx::simple_socket_client<socketxx::base_netsock> slave_sock = iosl_master::slave_connect(slave_id, 0, timeval{0,500000});
		slave_sock.o_bool(false);
		return true;
	} catch (...) {
		return false;
